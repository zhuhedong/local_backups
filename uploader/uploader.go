...
func NewUploader(filePath string, sliceBytes int) *Uploader {
	...
	err = common.ClientStoreMetadata(getUploadMetaFile(filePath), &metadata)
	if err != nil {
		fmt.Println("新建上传器失败")
		return nil
	}
...
func (u *Uploader) uploadSlice(part *FilePart) error {
	...
	fmt.Printf("上传分片文件成功，文件ID：%s，文件名：%s，序号：%d\n", part.Fid, part.FileName, part.Index)
	u.waitGoroutine.Done()
	return nil
}
...
func (u *Uploader) UploadFileBySlice() error {
	...
	if md5sum == "" {
		...
		// 保存md5到元数据文件
		u.Md5sum = md5sum
		err := common.ClientStoreMetadata(getUploadMetaFile(u.FilePath), &u.FileMetadata)
		if err != nil {
			return err
		}
	}
